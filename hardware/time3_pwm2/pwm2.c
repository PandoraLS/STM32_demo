...
#include "pwm2.h"
/*
 * 函数名：TIM3_GPIO2_Config
 * 描述  ：配置TIM3复用输出PWM时用到的I/O
 * 输入  ：无
 * 输出  ：无
 * 调用  ：内部调用
 */
static void TIM3_GPIO2_Config(void) 
{
  ...
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOC|RCC_APB2Periph_AFIO, ENABLE);  //使能GPIO外设和AFIO复用功能模块时钟
	...
	/*TIM3完全重映射的库函数实现方法*/
	GPIO_PinRemapConfig(GPIO_FullRemap_TIM3 , ENABLE); //重映射功能函数
...
/*
 * 函数名：TIM3_Mode2_Config
 * 描述  ：配置TIM3输出的PWM信号的模式，如周期、极性、占空比
 * 输入  ：无
 * 输出  ：无
 * 调用  ：内部调用
 */
static void TIM3_Mode2_Config(void)
{
