...
u8 NRF24L01_ReadBuf(u8 reg,u8 *pBuf,u8 bytes)
{
	...
	{	
		pBuf[byte_cnt] = NRF24L01_WriteReadByte(NOP); //从NRF24L01读取数据  
	}
...
u8 NRF24L01_WriteBuf(u8 reg ,u8 *pBuf,u8 bytes)
{
	...
	for(byte_cnt=0;byte_cnt<bytes;byte_cnt++)
	{
		NRF24L01_WriteReadByte(*pBuf++);	//写数据到缓冲区 	 
	}
...
void NRF24L01_RxMode(void)
{
	...
	NRF24L01_WriteReg(NRF_WRITE_REG+CONFIG1, 0x0f);  //配置基本工作模式的参数;PWR_UP,EN_CRC,16BIT_CRC,接收模式 
...
{  
	...
	NRF24L01_WriteReg(NRF_WRITE_REG+CONFIG1,0x0e);    //配置基本工作模式的参数;PWR_UP,EN_CRC,16BIT_CRC,发射模式,开启所有中断

	/*CE拉高，进入发送模式*/	
	NRF_CE_HIGH();
	NRF24L01_Delay(0xffff); //CE要拉高一段时间才进入发送模式
}
