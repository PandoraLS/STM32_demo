...
/* -----------------------------------------------------------------------
  ...
  TIM_OC1Init(TIM2, &TIM_OCInitStructure);	 //将TIM3指定的参数初始化外设TIM_OC1

  TIM_OC1PreloadConfig(TIM2, TIM_OCPreload_Enable);//使能TIM3在CCR1上的预装载寄存器，即
	//TIM3_CCR1的预装载值在更新事件到来时才能被传送至当前寄存器中
...
/* -----------------------------------------------------------------------
  ...
  TIM_OC1Init(TIM3, &TIM_OCInitStructure);	 //将TIM3指定的参数初始化外设TIM_OC1

  TIM_OC1PreloadConfig(TIM3, TIM_OCPreload_Enable);//使能TIM3在CCR1上的预装载寄存器，即
	//TIM3_CCR1的预装载值在更新事件到来时才能被传送至当前寄存器中
